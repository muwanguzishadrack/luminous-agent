...
/**
 * Phase 0 deliverable 0.7 (docs/90-roadmap.md): one realistic demo team —
 * exactly one team with 5 users in their five distinct roles, 1 WABA, 1 number,
 * 2,000 contacts with a consistent consent ledger, 30 conversations across all
 * four ownership states, 12 templates in every Meta status, 3 campaigns, 40
 * orders with payments in every ioTec status, CTWA referrals, labels and
 * canned replies.
 *
 * One team, because a user belongs to one team and a team holds one WABA and
 * one number (D-020) — the seed has to satisfy those constraints, not sidestep
 * them.
 *
 * Runs as the runtime role: Teams::initialize() is called right after the
 * team exists so Postgres RLS admits every insert. Bulk inserts bypass
 * Eloquent, so those rows carry team_id explicitly.
 */
class DemoTeamSeeder extends Seeder
{
    ...
    /**
     * The demo team id, set once the team exists.
     */
    ...
    public function run(): void
    {
        ...
        DB::transaction(function () {
            $team = $this->seedTeamAndUsers();

            $this->teamId = $team->id;

            [$waba, $number] = $this->seedMetaAssets();
            $this->seedContacts();
            $this->seedConsents();
            $templates = $this->seedTemplates($waba);
            $conversations = $this->seedConversations($number, $templates);
            $this->seedCtwaReferrals($conversations);
            $this->seedCampaigns($number, $templates);
            $this->seedCommerce($conversations);
            $this->seedLabels($conversations);
            $this->seedCannedReplies();
        });

        $this->command->info(sprintf('Demo team seeded in %.1fs', microtime(true) - $started));
    }

    /**
     * The one demo team, with all five users on it in their distinct roles.
     * Exactly one team: nobody gets a workspace of their own (D-020).
     */
    private function seedTeamAndUsers(): Team
    {
        ...
        // Every team-scoped insert from here requires context — RLS enforces
        // it, and the memberships below are the first ones.
        Teams::initialize($team);

        $people = [
            TeamRole::Owner->value => ['Demo Owner', 'demo@luminouscrm.test'],
            TeamRole::Admin->value => ['Amina Nakato', 'admin@luminouscrm.test'],
            TeamRole::Supervisor->value => ['Brian Okello', 'supervisor@luminouscrm.test'],
            TeamRole::Agent->value => ['Cissy Namuli', 'agent@luminouscrm.test'],
            TeamRole::Viewer->value => ['David Mugisha', 'viewer@luminouscrm.test'],
        ];

        foreach ($people as $role => [$name, $email]) {
            $this->users[$role] = User::factory()
                ->memberOf($team, TeamRole::from($role))
    ...
    /**
     * One WABA with a vaulted token and the team's single number (D-020).
     *
    ...
    private function seedMetaAssets(): array
    {
        ...
        $number = PhoneNumberFactory::new()->createOne([
     ...
     */
    private function seedConversations(PhoneNumber $phoneNumber, array $templates): array
    {
        $states = array_merge(
        ...
        $agents = [
            $this->users[TeamRole::Agent->value],
            $this->users[TeamRole::Supervisor->value],
        ];
    ...
    {
        $creator = $this->users[TeamRole::Admin->value];
    ...
    {
        $creator = $this->users[TeamRole::Supervisor->value];
     ...
     * Raw inserts bypass Eloquent, so every row must carry team_id itself
