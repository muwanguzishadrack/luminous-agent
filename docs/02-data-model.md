...
Postgres 17. Single database, single schema. **Every tenant-scoped table carries `tenant_id`** and
...
- Timestamps: `timestamptz`. Always store UTC; render in the tenant's WABA timezone.
...
`two_factor_*`. A user may belong to multiple tenants.

### `tenant_user`
| Column | Type | Notes |
|---|---|---|
| tenant_id / user_id | uuid | composite pk |
| role | enum | `owner`, `admin`, `supervisor`, `agent`, `viewer` |
| phone_number_ids | jsonb | null = all numbers; otherwise scope the user to specific numbers |
...
|---|---|---|
| id | uuid pk | |
| tenant_id | uuid index | tenant being impersonated |
...
|---|---|---|
| id | uuid pk | |
| tenant_id / waba_account_id | uuid index | |
...
Unique: `(tenant_id, waba_account_id, type)` where `revoked_at is null`.
...
|---|---|---|
| id | uuid pk | |
| tenant_id | uuid null | null until the tenant is created |
...
| tenant_id | uuid null index | resolved during processing, null if unresolvable |
...
Indexes: `(source, body_sha256) unique`, `(status, received_at)`, `(tenant_id, received_at)`.
...
| lifecycle_stage | varchar | `lead`, `engaged`, `customer`, `churned` — tenant-configurable |
...
| custom_fields | jsonb | tenant-defined schema |
| deleted_at | timestamptz null | soft delete |

Unique: `(tenant_id, wa_id)`. Index: `(tenant_id, last_inbound_at desc)`, GIN on `custom_fields`.
...
|---|---|---|
| id | bigint pk | |
| tenant_id / contact_id | uuid index | |
...
`tenant_id, contact_id, scope, state, source, occurred_at, consent_id` — updated by a listener on
...
`labels`: `id, tenant_id, name, color, kind (contact|conversation), created_by`. Pivots are simple.

### `notes`
`id, tenant_id, contact_id, conversation_id null, user_id, body, mentions jsonb, created_at`.
...
|---|---|---|
| id | uuid pk | |
| tenant_id / phone_number_id / contact_id | uuid index | |
...
Unique: `(tenant_id, phone_number_id, contact_id)`.
Indexes: `(tenant_id, state, last_message_at desc)`, `(assigned_user_id, state)`, `(csw_expires_at)`.

### `messages`
| Column | Type | Notes |
|---|---|---|
| id | uuid pk | |
| tenant_id / conversation_id | uuid index | |
...
Indexes: `wamid unique`, `(conversation_id, occurred_at)`, `(tenant_id, occurred_at desc)`,
...
`id bigint, tenant_id, message_id, wamid, status, error_code null, pricing jsonb null, occurred_at, payload jsonb`.
...
`id bigint, tenant_id, conversation_id, event (pass|take|request|app_roles), previous_owner_app_id, new_owner_app_id, metadata jsonb, actor_type, actor_id, occurred_at`.

### `canned_replies`
`id, tenant_id, shortcut, title, body, variables jsonb, is_shared, created_by`.
...
|---|---|---|
| id | uuid pk | |
| tenant_id / waba_account_id | uuid | |
...
`id, tenant_id, key, name` + `templates.template_group_id`. Lets a campaign target a group and
resolve the right language per contact.

### `template_events`
Append-only from `message_template_status_update`, `_quality_update`, `_components_update`,
`template_category_update`: `id, tenant_id, template_id, event, from, to, reason, payload, occurred_at`.
...
|---|---|---|
| id | uuid pk | |
| tenant_id / phone_number_id | uuid | |
| name | varchar | |
...
|---|---|---|
| id | bigint pk | |
| tenant_id / campaign_id / contact_id | index | |
...
|---|---|---|
| id | uuid pk | |
| tenant_id / phone_number_id | uuid unique | one agent per number |
...
`id, tenant_id, mba_agent_id, wa_id, added_by, added_at, removed_at null`.

### `mba_knowledge_sources`
| Column | Type | Notes |
|---|---|---|
| id | uuid pk | |
| tenant_id / mba_agent_id | uuid | |
...
| name / base_url / auth_scheme | | our own connector endpoints, per-tenant |
...
### `connector_tokens`
Bearer tokens Meta uses to call **us**. `id, tenant_id, name, token_hash, prefix, abilities jsonb, last_used_at, expires_at, revoked_at`. Store only a hash.

### `mba_events`
Agent Events we push: `id, tenant_id, conversation_id, kind, payload jsonb, external_id, status, sent_at, error`.

### `mba_evals`
`id, tenant_id, mba_agent_id, kind (test|eval), request jsonb, result jsonb, score numeric, run_at, run_by`.
...
Unique: `(tenant_id, retailer_id)`.

### `orders`
| Column | Type | Notes |
|---|---|---|
| id | uuid pk | |
| tenant_id / contact_id / conversation_id | uuid index | |
...
|---|---|---|
| id | uuid pk | |
| tenant_id / order_id | uuid index | order_id nullable (standalone collection) |
| contact_id | uuid null | |
| direction | enum | `collection`, `disbursement` |
| provider | varchar | `iotec` |
| **external_id** | varchar **unique per tenant** | **our ULID** sent to ioTec as `externalId`. ioTec does not guarantee uniqueness — we do. |
...
Indexes: `(tenant_id, status)`, `(order_id)`, `(provider_id)`, `(status, last_polled_at)` for the poller.

### `payment_events` — append-only
`id bigint, tenant_id, payment_id, status, status_code, status_message, source (callback|poll|manual), raw jsonb, occurred_at, received_at`.
...
### `iotec_wallets`
`id, tenant_id null, iotec_wallet_id uuid, name, currency, actual_balance_minor, available_balance_minor, collection_callback_url, disbursement_callback_url, callback_header_name, callback_header_value (encrypted cast), last_synced_at`.
`tenant_id` is null for our own platform wallet. The callback header pair is what ioTec sends on
...
### `conversions`
What we report back to Meta. `id, tenant_id, contact_id, order_id null, event_name (Purchase|Lead|AddToCart|InitiateCheckout), value_minor, currency, ctwa_clid, event_time, dedup_key unique, status (pending|sent|failed), response jsonb, sent_at`.
...
| occurred_on | date index | tenant WABA timezone day |
| message_id / campaign_id | uuid null | traceability |
| created_at | | |

Index: `(tenant_id, occurred_on, meter)`.
...
Unique: `(effective_from, region, category, tier_min)`. Global, not tenant-scoped. Cost resolution
picks the row effective at `usage_meters.occurred_on` — never retroactively repriced.

### `wallet_entries` — tenant billing ledger, append-only
`id bigint, tenant_id, kind (topup|charge|adjustment|refund), amount_minor (signed), currency, balance_after_minor, reference_type, reference_id, description, created_at`.
...
`id, tenant_id, waba_account_id, field (analytics|conversation_analytics|pricing_analytics|template_analytics|template_group_analytics), granularity, start_at, end_at, dimensions jsonb, dimensions_hash char(64) (sha256 of the canonicalised dimensions json — computed in the model, stored, indexed), payload jsonb, fetched_at`.
...
### `health_events`
`id bigint, tenant_id, phone_number_id null, kind, severity (info|warning|critical), payload jsonb, occurred_at, acknowledged_at null, acknowledged_by null`.
...
| Group | Tables |
|---|---|
| Tenancy & identity | 5 — tenants, users, tenant_user, audit_logs, admin_sessions |
...
1. Tenancy & identity → 2. Meta assets → 3. Webhook ingest → 4. Contacts & consent →
